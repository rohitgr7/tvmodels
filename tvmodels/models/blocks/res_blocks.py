...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        out = self.net(x)
        if self.downsample is not None: out += self.downsample(x)
        else: out += x
...
class ResNetBlock(Bottleneck):
    ...
    def __init__(self, ni, nf, stride, downsample=None, se=False, ratio=16, groups=1, base_width=64):
        ...
        if se: layers += [SEBlock(nf * 4, nf * 4 // ratio)]
        self.net = nn.Sequential(*layers)
        self.relu = relu()
        self.downsample = downsample
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, base_blocks, ni=64, ps=0., init_ksz3=False, down_ksz=1, down_pad=0,
                 nc=1000, se=False, **kwargs):
        super().__init__()
        ...
        else: layers += [*conv_bn(3, ni, 7, 2, 3, bn=True), relu()]
    ...
    def _res_blocks(self, block, nf, nblocks, down_ksz=1, down_pad=0, stride=1, **kwargs):
        ...
        layers += [block(self.ni, nf, 1, se=self.se, **kwargs) for _ in range(1, nblocks)]
...
class SENetBlock(Bottleneck):
    ...
    def __init__(self, ni, nf, stride, downsample=None, groups=64, ratio=16, **kwargs):
        ...
        layers = [*conv_bn(ni, nf * 2, 1, bn=True), relu(),
                  *conv_bn(nf * 2, nf * 4, 3, stride, 1, groups=groups, bn=True), relu(),
